...
enum class AnimType {
    manual, forward, forward_stop, forward_cycle, forward_backward_cycle
};
...
public:
    ...
    void
    create(const std::string &name, const sf::Texture &texture, const Vector &off_set, const Vector &size, int cols,
           int rows, float speed, AnimType anim_type = AnimType::forward_cycle);

    void create(const std::string &name, const sf::Texture &texture, const Rect &rect);

    void
    create(const std::string &name, const sf::Texture &texture, const std::vector<sf::IntRect> &rects, float speed);
...
enum class ECollisionTag : int {
    none = 0, cell = 1, floor = 2, left = 4, right = 8, up = cell, down = floor
};
