...
void CBlocks::draw(sf::RenderWindow *render_window) {
    ...
    view_rect = Rect(toBlockCoordinates(center - size / 2), toBlockCoordinates(size)).getIntersection(
            Rect(0, 0, cols(), rows()));
...
Vector CBlocks::traceLine(const Vector &start_cell, const Vector &direction) {
    return m_map->traceLine(start_cell, direction,
                            [](AbstractBlock *block) { return !(block && block->isColliable()); });
}
...
Vector CBlocks::collsionResponse(const Rect &body_rect, const Vector &body_speed, float delta_time,
                                 ECollisionTag &collision_tag) {
    CBlocks *blocks = this;
    ...
    for (int x = new_pos.x / tile_size; x < (new_pos.x + own_size.x) / tile_size; ++x)
        for (int y = new_pos.y / tile_size; y < (new_pos.y + own_size.y) / tile_size; ++y)
            if (blocks->isBlockInBounds(x, y) &&
                (blocks->isCollidableBlock(x, y) || (blocks->isInvizibleBlock({x, y}) && body_speed.y < 0))) {
                if (body_speed.y == 0 &&
                    body_rect.bottom() > (y + 1) * tile_size) //fix for "body in the restricted area" collision bug
                {
...
COneBrick::COneBrick(const Vector &pos, const Vector &speed_vector) {
    ...
    m_sprite_sheet.load(*MarioGame().textureManager().get("Items"), {{96, 0,  16, 16},
                                                                     {96, 16, 16, -16}});
...
CTwistedCoin::CTwistedCoin(const Vector &pos, CMario *mario) {
    ...
    m_animator.create("shine", *MarioGame().textureManager().get("Items"), Vector(0, 116), Vector(40, 32), 5, 1, 0.01f,
                      AnimType::forward);
