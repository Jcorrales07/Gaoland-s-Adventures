...
CGameObject *CGameObject::findObjectByName(const std::string &name) {
    auto it = std::find_if(m_objects.begin(), m_objects.end(),
                           [this, &name](const CGameObject *obj) -> bool { return obj->getName() == name; });
    if (it != m_objects.end())
        return *it;
    return nullptr;
}
    ...
    for (int y = 0; y < rows; ++y)
        for (int x = 0; x < cols; ++x)
            m_sprites.emplace_back(texture,
                                   sf::IntRect(x * std::abs(size.x) + off_set.x, y * std::abs(size.y) + off_set.y,
                                               size.x, size.y));
    setSpriteIndex(0);
...
void Pallete::create(const std::initializer_list<sf::Color> &original_colors,
...
void Animator::create(const std::string &name, const sf::Texture &texture, const Vector &off_set, const Vector &size,
                      int cols, int rows, float speed, AnimType anim_type) {
...
void Animator::create(const std::string &name, const sf::Texture &texture, const std::vector<sf::IntRect> &rects,
                      float _speed) {
    CSpriteSheet *animation = new CSpriteSheet();
...
void CLabel::draw(sf::RenderWindow *window) {
    ...
    if (!m_text.getString().isEmpty()) {
        if (m_text_align == center)
            m_text.setPosition(getPosition() + m_rect.size() / 2 -
                               Vector(m_text.getGlobalBounds().width, m_text.getGlobalBounds().height) / 2);
...
Vector
collsionResponse(const Rect &own_rect, const Vector &own_speed, const Rect &other_rect, const Vector &other_speed,
