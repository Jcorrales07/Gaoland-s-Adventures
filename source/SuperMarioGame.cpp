...
CMarioGame::CMarioGame() : CGame("SuperMario", {1280, 720}) {
    // Load textures
    const std::string textures_dir = MARIO_ASSETS_PATH + "Textures/";
    ...
    // Load music
    const std::string music_dir = MARIO_ASSETS_PATH + "Music/";
...
void CMarioGame::loadLevel(const std::string &_level_name) {
    ...
    setScene(new CMarioGameScene(MARIO_ASSETS_PATH + "Levels/" + level_name + ".tmx"));
    m_gui_object->moveToFront();
    m_current_stage_name = level_name;
}

void CMarioGame::loadSubLevel(const std::string &sublevel_name) {
    pushScene(new CMarioGameScene(MARIO_ASSETS_PATH + "Levels/" + sublevel_name + ".tmx"));
    m_gui_object->moveToFront();
}
...
CBlocks *parseBlocks(tinyxml2::XMLElement *element) {
    ...
    static auto blocks_fabric = [](char c) -> AbstractBlock * {
        ...
        switch (c) {
            case (0):
                return NULL;
            case (AbstractBlock::BRICK_TILE_CODE):
                return new CBricksBlock();
            case (AbstractBlock::MONEY_TILE_CODE):
                return new CMoneyBox();
            case (AbstractBlock::COIN_TILE_CODE):
                return new CQuestionBlock<CTwistedCoin>(AbstractBlock::COIN_TILE_CODE);
            case (AbstractBlock::MUSHROOM_TILE_CODE):
                return new CQuestionBlock<CMushroom>(AbstractBlock::MUSHROOM_TILE_CODE);
            case (AbstractBlock::BRICK_MUSHROOM_TILE_CODE):
                return new CQuestionBlock<CMushroom>(AbstractBlock::BRICK_MUSHROOM_TILE_CODE, NOT_INVIZ_STYLE,
                                                     BRICK_STYLE);
            case (AbstractBlock::LADDER_TILE_CODE):
                return new CQuestionBlock<CLadder>(AbstractBlock::LADDER_TILE_CODE);
            case (AbstractBlock::INVIZ_LADDER_TILE_CODE):
                return new CQuestionBlock<CLadder>(AbstractBlock::INVIZ_LADDER_TILE_CODE, INVIZ_STYLE);
            case (AbstractBlock::BRICK_LADDER_TILE_CODE):
                return new CQuestionBlock<CLadder>(AbstractBlock::BRICK_LADDER_TILE_CODE, NOT_INVIZ_STYLE, BRICK_STYLE);
            case (AbstractBlock::INIZ_UP_TILE_CODE):
                return new CQuestionBlock<COneUpMushroom>(AbstractBlock::INIZ_UP_TILE_CODE, INVIZ_STYLE);
            case (AbstractBlock::INIZ_COIN_CODE):
                return new CQuestionBlock<CTwistedCoin>(AbstractBlock::INIZ_COIN_CODE, INVIZ_STYLE);
            case (AbstractBlock::LIVE_UP_CODE):
                return new CQuestionBlock<COneUpMushroom>(AbstractBlock::LIVE_UP_CODE, NOT_INVIZ_STYLE, BRICK_STYLE);
            case (AbstractBlock::STAR_CODE):
                return new CQuestionBlock<CStar>(AbstractBlock::STAR_CODE, NOT_INVIZ_STYLE, BRICK_STYLE);
            default:
                return new CStaticBlock(c);
        }
    };
...
CMarioGUI::CMarioGUI() {
    ...
    m_fire_pallete.create({sf::Color(202, 77, 62), sf::Color(132, 133, 30)},
                          {sf::Color(255, 255, 255), sf::Color(202, 77, 62)});
    ...
    m_menu_selector = new CLabel(
            sf::Sprite(*CMarioGame::instance()->textureManager().get("Items"), {128, 150, 32, 32}));
